...
`init` picks your accent color, derives all 9 tokens, writes `aqus.css`, injects CSS imports into your entry point, and optionally installs the Aqus design agent for Claude Code.

## Manual setup

Skip this if you ran `npx aqus init` — it handles everything below automatically.
...
```css
:root {
  --accent-h:      265;              /* hue only — chart palette derives from this */
...
Derivation formula: [`src/tokens/ACCENT.md`](../src/tokens/ACCENT.md).
